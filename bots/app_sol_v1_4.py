# app_sol_render.py — Apex Omni SOL Bot (v1.4) — Render-ready
...
# ---------------- Bot Identity & Capital Manager ----------------
BOT_ID = "SOL"  # must match Capital Manager config
CAPMGR_URL = os.environ.get("CAPMGR_URL", "http://127.0.0.1:5015")

# ---------------- Config (SOL) ----------------
APEX_SYMBOL        = "SOL-USDT"
...
# Trade gates
USE_BIAS            = os.environ.get("USE_BIAS", "0") == "1"
ALLOW_COUNTER_TREND = os.environ.get("ALLOW_COUNTER_TREND", "1") == "1"

# Exchange tick/steps (ApeX SOL)
TICK_SIZE       = Decimal("0.01")   # SOL price tick
SIZE_STEP       = Decimal("0.1")    # SOL size step
...
CT_SL_PCT       = Decimal(os.environ.get("CT_SL_PCT", "0.0075"))
...
def floor_to_step(size: Decimal) -> Decimal:
    return (_D(size) / SIZE_STEP).to_integral_value(rounding=ROUND_DOWN) * SIZE_STEP
    ...
    try:
        ...
        for k in ("last", "close"):
            if t.get(k) is not None:
                return _D(t.get(k))
        info = t.get("info") or {}
        for k in ("lastPrice", "markPrice", "indexPrice", "price"):
            v = _D_safe(info.get(k))
            if v: return v
    except Exception:
        pass
    return None
...
# ---------------- Bias via Binance (1h) — hardened with sticky TTL ----------------
...
def place_market_order(direction: str, trade_size_pct: Optional[Decimal] = None) -> Optional[str]:
    try:
        ...
        # Price & STRICT sizing (do not exceed reservation)
        mark_price = get_public_price()
        mark_price_r = floor_to_tick(mark_price)
        price_str   = fmt_price(mark_price_r)

        allowed_notional = approved * LEVERAGE
        size = floor_to_step(allowed_notional / mark_price_r)
        size_min = ceil_to_step(MIN_ORDER_USDT / mark_price_r)
        if size < SIZE_STEP or size < size_min:
            dash("warn", "Approved margin too small after rounding/min-notional",
                 extra={"approved_margin": str(approved)})
        ...
        if not order_id:
            dash("error", "Entry rejected by exchange", extra={"resp": resp})
            capmgr_release(approved)
            return None

        # Free any unused reservation due to floor rounding
        used_margin_est = (size * mark_price_r) / LEVERAGE
        unused = approved - used_margin_est
        if unused > Decimal("0.0001"):
            capmgr_release(unused)
            dash("state", "Released unused reserved margin", extra={"unused": str(unused)})
        ...
        with POSITION_LOCK:
            POSITION.update({
                ...
                "tp": tp_price, "sl": sl_exec, "margin": used_margin_est, "reserved_margin": used_margin_est
            })
        STATE["mf_flip_since_entry"] = False
...
def test_force_entry():
    if request.method == "GET":
        return jsonify({"ok": True, "hint": 'POST {"side":"LONG|SHORT","trade_pct":0.12}'})
...
# Boot immediately unless explicitly disabled (Gunicorn path)
